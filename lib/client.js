...
`;
		...
		function commitLayout(whaleLeft) {
			...
			if (!input || inputWidth <= 0 || !convEl) {
				try { console.debug("[pp-layout] pending (no input/col)", { input: !!input, inputWidth, hasCol: !!convEl, t: Math.round(performance.now()) }); } catch (e) {}
				return false;
			}
			...
			try { console.debug("[pp-layout] commit", { whaleLeft: Math.round(whaleLeft), reserve: Math.round(reserve), inputWidth: Math.round(inputWidth), convLeft: Math.round(convLeft), convMargin: Math.round(convMargin), panelLeft: Math.round(panelLeft), t: Math.round(performance.now()) }); } catch (e) {}
		...
		function layoutPanel() {
			...
			if (whale) {
				...
				if (isFinite(rect.left) && rect.left > 0 && rect.left < (document.documentElement.clientWidth || window.innerWidth) * 0.98) {
					whaleLeft = rect.left;
				}
			}
			// settled measured edge -> commit it once. Changing edge -> hold
			// (retry next frame) so we never write a transient margin, but a
			// bounded deadline (~8 frames) forces the stable viewport
			// fallback instead of waiting forever on a jittery decoration.
			// No skin -> commit the stable viewport fallback immediately.
			var anchor = 0;
			if (whaleLeft > 0) {
				if (whaleLeftSeen > 0 && Math.abs(whaleLeft - whaleLeftSeen) <= 2) {
					anchor = whaleLeft; // settled: commit final value
				} else {
					whaleLeftSeen = whaleLeft;
					layoutSampleCount += 1;
					try { console.debug("[pp-layout] sampling", { whaleLeft: Math.round(whaleLeft), seen: Math.round(whaleLeftSeen), n: layoutSampleCount, t: Math.round(performance.now()) }); } catch (e) {}
					if (layoutSampleCount < 8) {
						if (!layoutScheduled) {
					...
					// deadline reached: commit the stable fallback (once)
				}
			}
			layoutSampleCount = 0;
			if (commitLayout(anchor) !== true) {
				// prerequisites not met (input not mounted yet): retry next frame
				if (!layoutScheduled) {
					layoutScheduled = true;
					requestAnimationFrame(function () {
						layoutScheduled = false;
		...
		function renderPanel() {
			...
			// fresh layout pass for this session: reset sampling state so a
			// stale whale measurement from a previous session cannot skip the
			// settle check (and thus write a transient margin)
			whaleLeftSeen = 0;
			layoutSampleCount = 0;
			layoutPanel();

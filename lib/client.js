...
/* Panel appears/disappears with a fade+slide transition instead of a hard
   display toggle. The conversation column must NOT animate its margin: DSH
   gives centerCol a global transition:all, and the content is centered
   inside the column, so animating the margin makes the chat drift
   (center -> left -> center) while the two centering rules fight. Pin it. */
body[data-pair-panel-mode='abg'] .pp-card { display:flex; animation: pp-panel-in .32s cubic-bezier(.22,.78,.2,1); }
...
`;
		...
		function layoutPanel() {
			...
			if (whale && whale.tagName === "IMG" && !whale.complete) {
				// Do NOT lay out with an unstable whale rect: its left edge
				// changes as the image loads, so a pre-load margin would
				// differ from the post-load margin and the column would jump
				// (content drifts left, then back). Wait for the load event,
				// then lay out exactly once with final geometry.
				if (!runtime.whaleLoadPending) {
			...
			// Without the maid-atelier skin there is no whale to anchor to;
			// fall back to a fixed right-side reserve so the panel still sits
			// beside the conversation. With the skin, anchor to the real maid.
			var whaleLeft = whale ? whale.getBoundingClientRect().left : 0;
			if (!whale) {
				whaleLeft = Math.max(viewport - 480, viewport * 0.7);
			} else if (!isFinite(whaleLeft) || whaleLeft <= 0 || whaleLeft > viewport * 0.98) {
				// maid not laid out yet: keep the original layout untouched
				return;
			}
			// Geometry stability gate: if the whale's left edge is still
			// changing between passes, defer the margin write. The 2.5s poll
			// and the resize listener will retry once it settles, so the
			// column only ever moves to its final position (no drift-back).
			if (whale && Math.abs(whaleLeft - whaleLeftSeen) > 2) {
		...
		function renderPanel() {
			...
			if (!(isAbg() && runtime.enabled && !isNewSession())) { document.body.removeAttribute("data-pair-panel-mode"); clearLayout(); restoreLeftMaid(); panel.replaceChildren(); runtime.feed = null; return; }

...
`;
		...
		// The right maid image reports complete=true before its final decoded
		// size is stable (e.g. 409px -> 384px after layout). Laying out once
		// against an unstable rect writes a wrong margin that DSH later
		// overrides, making the chat drift. Track the measured left edge and
		// only treat the geometry as stable when it stops changing.
		var whaleLeftSeen = 0;
		...
		function commitLayout(whaleLeft) {
			...
			if (!input || inputWidth <= 0 || !convEl) return false;

...
`;
		...
		// sidebar) but is shrunk so she never covers the conversation bubbles
		// OR the input box. The input is centered in the column, so its left
		// edge is fixed relative to the column; the maid must end before it.
		// Her art is 1394x2649 (~0.526 aspect); height drives width.
		var LEFT_MAID_ASPECT = 1394 / 2649; // width per height unit
		function adjustLeftMaid() {
			...
			maid.style.left = "";
			maid.style.bottom = "";
			// dynamic size: keep her as large as possible (max 800px tall)
			// without letting her right edge reach the input's left edge.
			var height = Math.min(800, Math.max(240, window.innerHeight * 0.62));
			var maidLeft = maid.getBoundingClientRect().left;
			var input = document.querySelector('textarea, [contenteditable="true"]');
			if (input && isFinite(maidLeft) && maidLeft > 0) {
				var ir = input.getBoundingClientRect();
				if (isFinite(ir.left) && ir.left > maidLeft) {
					var maxRight = ir.left - 16;
					var maxWidth = maxRight - maidLeft;
					var maxHeightByInput = maxWidth / LEFT_MAID_ASPECT;
					height = Math.min(height, Math.max(200, maxHeightByInput));
				}
			}
			maid.style.height = height + "px";
		}

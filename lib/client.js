...
`;
		...
		// reserve (Codex: the whale is decoration, never a layout gate). The
		// maid measurement is only an optional refinement with a short
		// deadline; if it jitters or loads late, the stable fallback already
		// committed. Panel visibility is committed atomically AFTER the
		// margin/position write (pending -> commit -> ready), so there is no
		// visible intermediate state (panel stuck at right:0, column not yet
		// giving way).
		...
		function layoutPanel() {
			...
			// Commit ONCE with stable geometry. Never write the margin twice:
			// a fallback commit followed by a refinement commit makes the
			// chat drift left then back. Instead sample a few animation
			// frames until the whale's left edge stops changing (or a short
			// deadline passes), then commit a single final value.
			if (!whale || whaleLeft <= 0) {
				commitLayout(0); // no skin: stable viewport fallback
				return;
			}
			if (Math.abs(whaleLeft - whaleLeftSeen) > 2) {
				whaleLeftSeen = whaleLeft;
				// still settling: resample on the next frame, bounded
				if (!runtime.whaleSampling) {
					runtime.whaleSampling = true;
					var samples = 0;
					var trySettle = function () {
						samples += 1;
						if (!(isAbg() && runtime.enabled)) { runtime.whaleSampling = false; return; }
						var r2 = whale.getBoundingClientRect();
						var w2 = isFinite(r2.left) && r2.left > 0 ? r2.left : 0;
						if (Math.abs(w2 - whaleLeftSeen) <= 2 || samples >= 6) {
							runtime.whaleSampling = false;
							whaleLeftSeen = w2;
							commitLayout(w2 > 0 ? w2 : 0);
							return;
						}
						whaleLeftSeen = w2;

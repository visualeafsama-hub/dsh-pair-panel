...
`;
		...
		function layoutPanel() {
			...
			// stable fallback commit immediately; optional refinement when the
			// maid measurement is available and stops jittering
			...
			if (Math.abs(whaleLeft - whaleLeftSeen) > 2) {
				...
				// first observation: commit with fallback now, refine later
				if (!layoutReady) commitLayout(0);
				return;
			}
			whaleLeftSeen = whaleLeft;

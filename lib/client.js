...
/* Panel visibility is atomic with layout: hidden until geometry commits
   (data-pp-ready), then fades/slides in. Off state animates out too. */
body[data-pair-panel-mode='abg'] .pp-card { display:flex; visibility:hidden; }
body[data-pair-panel-mode='abg'] .pp-card[data-pp-ready] { visibility:visible; animation: pp-panel-in .32s cubic-bezier(.22,.78,.2,1); }
@keyframes pp-panel-in { from { opacity:0; transform: translateX(18px); } to { opacity:1; transform:none; } }
.pp-card.pp-hide { visibility:visible !important; animation: pp-panel-out .28s cubic-bezier(.22,.78,.2,1) forwards; }
@keyframes pp-panel-out { from { opacity:1; transform:none; } to { opacity:0; transform: translateX(18px); } }
body[data-pair-panel-mode='abg'] [class*='centerCol'], body:not([data-pair-panel-mode]) [class*='centerCol'] { transition: none !important; }
body[data-pair-panel-mode='abg'] [data-maid-character='left'], body:not([data-pair-panel-mode]) [data-maid-character='left'] { transition: left .32s ease, translate .32s ease, height .32s ease; }
...
.pp-msg.user { align-self:flex-start; background:#e3e9f7; border:1px solid rgba(82,106,168,.3); }
...
.pp-task { align-self:flex-start; color:#6b78a3; font-size:11px; }
...
`;
		...
		// A brand-new session has no history: the selected session row is the
		// "新会话" placeholder. The panel stays off there (original layout,
		// original maid sizes) without touching the user's stored preference,
		// so switching back to a real session restores the panel as before.
		// IMPORTANT: when no selected row exists yet (page still mounting),
		// treat the session as new -> panel stays off until the UI confirms
		// a real session. Otherwise the panel flashes on (column shifts left)
		// and then off again on every reload.
		function isNewSession() {
			try {
				var selected = document.querySelector("[class*='sessionRow'][class*='selected']");
				if (!selected) return true;
				var text = (selected.textContent || "").trim();
				return text === "新会话" || text.indexOf("新会话") === 0;
		...
		// The left maid keeps her skin position and ORIGINAL size. She must
		// not change size when the panel is toggled (user requirement), so
		// no dynamic shrinking here — restore her native geometry.
		...
		// Primary layout is computed from the viewport + a stable right-side
		// reserve (Codex: the whale is decoration, never a layout gate).
		// commitLayout returns false and stays pending unless the layout
		// prerequisites are ALL met — an input box that is not mounted yet
		// (width 0) must never produce a fallback margin, otherwise the
		// margin gets written twice (empty -> fallback -> real) and the chat
		// drifts. Panel visibility is committed atomically AFTER the write
		// (pending -> commit -> ready), so there is no visible intermediate.
		var DEFAULT_RIGHT_RESERVE = 610; // px the right maid occupies at 2560w
		var LEFT_MAID_ASPECT = 1394 / 2649; // width per height unit (same art)
		var layoutReady = false;

		// layout diagnostics: appended to window.__ppLayoutLog AND console so
		// both the browser console and CDP can read exact values
		function ppLog(tag, obj) {
			try {
				var line = "[pp-layout] " + tag + " " + JSON.stringify(obj);
				console.debug(line);
				if (!window.__ppLayoutLog) window.__ppLayoutLog = [];
				window.__ppLayoutLog.push(line);
				if (window.__ppLayoutLog.length > 200) window.__ppLayoutLog.shift();
			} catch (e) {}
		}

		function commitLayout(whaleLeft) {
			...
			// Codex: never write a margin against a not-yet-mounted input.
			// Stay pending; renderPanel retries on later frames.
			if (!input || inputWidth <= 0 || !convEl) {
				ppLog("pending", { input: !!input, inputWidth: inputWidth, hasCol: !!convEl, t: Math.round(performance.now()) });
				return false;
			}
			var convLeft = convEl.getBoundingClientRect().left;
			if (!isFinite(convLeft) || convLeft <= 0) convLeft = 280;
			// reserve: measured maid left edge when available and sane, else
			// the stable viewport-based fallback
			var reserve = whaleLeft > 0 ? viewport - whaleLeft : DEFAULT_RIGHT_RESERVE;
			var panelWidth = Math.min(448, Math.max(240, viewport - reserve - convLeft - inputWidth - LAYOUT_GAP));
			var panelLeft = Math.max(0, viewport - reserve - panelWidth - PANEL_SHIFT);
			var convMargin = viewport + convLeft + inputWidth - 2 * panelLeft + 2 * LAYOUT_GAP;
			convMargin = Math.max(0, convMargin);
			var right = Math.max(0, viewport - panelLeft - panelWidth);
			ppLog("commit", { whaleLeft: Math.round(whaleLeft), reserve: Math.round(reserve), inputWidth: Math.round(inputWidth), convLeft: Math.round(convLeft), convMargin: Math.round(convMargin), panelLeft: Math.round(panelLeft), t: Math.round(performance.now()) });
			...
			adjustLeftMaid();
			layoutApplied = true;
			// commit: now that geometry is written, reveal the panel (fade in)
			if (panel && !layoutReady) {
				layoutReady = true;
				requestAnimationFrame(function () {
					panel.setAttribute('data-pp-ready', '1');
				});
			}
			return true;
		}

		// Layout scheduling: commitLayout returns false while prerequisites
		// (input mounted, column present) are unmet; renderPanel retries on
		// animation frames until the FIRST successful commit. The whale is a
		// decoration: the margin is written EXACTLY ONCE with the settled
		// measured edge (or the stable fallback when no skin). Never commit
		// twice — a fallback write followed by a refined write is what made
		// the chat drift left then back.
		var layoutScheduled = false;

		// The right maid's FINAL (active-phase) geometry is a deterministic
		// skin contract — the same rules the maid-atelier skin applies to
		// [data-maid-character='right'] under [data-phase='active']:
		//   right: clamp(-70px, -3vw, -24px); height: clamp(420px, 62vh, 730px)
		// Compute the settled left edge from those rules instead of measuring
		// the live rect: the live rect lingers at the hero size (1965px) for
		// seconds before the skin applies the active rules (2246px), so any
		// measurement-based anchor commits a transient margin (chat drifts).
		// Like the new-session layout, the old-session layout is then a fixed
		// deterministic value — commit once, panel waits at the final spot.
		function activeWhaleLeft(viewport, vh) {
			var right = Math.max(-70, Math.min(-24, vh * -0.03)); // clamp(-70,-3vw,-24)
			var height = Math.max(420, Math.min(730, vh * 0.62)); // clamp(420,62vh,730)
			var width = height * LEFT_MAID_ASPECT;
		...
		function layoutPanel() {
			...
			var viewport = document.documentElement.clientWidth || window.innerWidth;
			var vh = window.innerHeight || viewport * 0.5625;
			var whale = document.querySelector("[data-maid-character='right']");
			// deterministic active-phase anchor when the skin is present;
			// stable viewport fallback otherwise
			var whaleLeft = whale ? activeWhaleLeft(viewport, vh) : 0;
			// commit exactly once; retry next frame only while prerequisites
			// (input mounted, column present) are unmet
			if (commitLayout(whaleLeft) !== true) {
				if (!layoutScheduled) {
					layoutScheduled = true;
					requestAnimationFrame(function () {
						layoutScheduled = false;
						if (isAbg() && runtime.enabled && !isNewSession()) layoutPanel();
					});
				}
			}
		}
		...
		// Feed-only refresh: renders panel content without touching layout.
		// Called by the 2.5s poll; layout must NOT run from the poll (it would
		// rewrite the margin whenever the maid edge changes -> chat drift).
		function updateFeed() {
			...
			// (response_item + event_msg); show each unique message once.
			{
				...
				for (var fi = 0; fi < feed.length; fi++) {
					...
					for (var si = Math.max(0, seenFeed.length - 4); si < seenFeed.length; si++) {
						var prev = seenFeed[si];
						if (prev.key === itemKey) { isDup = true; break; }
						// task_complete repeats the last agent message but is
						// truncated differently; dedupe by prefix too.
						if (item.kind === "task" && prev.item.kind === "message" && prev.item.text && itemText &&
							(prev.item.text.indexOf(itemText) === 0 || itemText.indexOf(prev.item.text) === 0)) { isDup = true; break; }
					}
		...
		function renderPanel() {
			var panel = runtime.panel; if (!panel) return;
			var visible = isAbg() && runtime.enabled && !isNewSession();
			if (!visible) {
				// play the hide animation while the panel is still shown,
				// then clean up layout/state after it finishes
				if (panel.hasAttribute('data-pp-ready') && !panel.classList.contains('pp-hide')) {
					panel.removeAttribute('data-pp-ready');
					panel.classList.add('pp-hide');
					var p = panel;
					setTimeout(function () {
						p.classList.remove('pp-hide');
						// if the user re-enabled during the hide animation,
						// renderPanel already ran the show path; don't clobber
						if (!(isAbg() && runtime.enabled && !isNewSession())) {
							document.body.removeAttribute("data-pair-panel-mode"); clearLayout(); restoreLeftMaid(); p.replaceChildren(); runtime.feed = null; layoutReady = false;
						}
					}, 300);
					return;
				}
				document.body.removeAttribute("data-pair-panel-mode"); clearLayout(); restoreLeftMaid(); panel.replaceChildren(); runtime.feed = null; layoutReady = false;
				return;
			}
			document.body.setAttribute("data-pair-panel-mode", "abg");
			panel.classList.remove('pp-hide');
			layoutPanel();
			updateFeed();
		}
		...
		function Sensor(props) {
			...
			React.useEffect(function () { if (!runtime.dsh || runtime.dsh.used !== used || runtime.dsh.window !== windowSize) { runtime.dsh = { used: used, window: windowSize }; updateFeed(); } }, [used, windowSize]);
			if (!isAbg() || isNewSession()) return null;
			...
			ctx.effect(function () {
				...
				function tick() {
					fetch("/pair-panel/snapshot", { cache:"no-store" })
						...
						.then(function (data) {
							if (disposed) return;
							var firstAbg = !(runtime.snapshot && runtime.snapshot.ok && runtime.snapshot.mode === "abg") && data.ok && data.mode === "abg";
							runtime.snapshot = data; runtime.error = null;
							if (firstAbg) {
								// first confirmation of abg mode: run the full
								// show+layout path once
								renderPanel();
							} else {
								// feed content only; never re-run layout from
								// the poll — that rewrote the margin whenever
								// the maid edge changed and made the chat drift
								updateFeed();
							}
							notify();
						})
						.catch(function (error) { if (disposed) return; runtime.snapshot = null; runtime.error = String((error && error.message) || error); updateFeed(); notify(); });
				}
				tick(); var timer = window.setInterval(tick, POLL_MS);
				var onResize = function () { if (isAbg() && runtime.enabled) layoutPanel(); };
				window.addEventListener("resize", onResize);
				// React when the active session's "is new?" state flips (e.g. to
				// a brand-new one). Only the boolean matters, not the row text:
				// the label changes several times mid-switch, and each change
				// used to rewrite the column margin (causing layout bounce).
				var lastIsNew = null;
				var sessionObserver = new MutationObserver(function () {
					var nowNew = isNewSession();
					if (nowNew === lastIsNew) return;
					lastIsNew = nowNew;
					renderPanel(); notify();
				});
				sessionObserver.observe(document.body, { subtree: true, childList: true, characterData: true });
				return function () {

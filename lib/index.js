...
// Instance ground truth: `abg dsh --pair <name>` writes which pair THIS DSH
// serves into ~/agentbridge-dsh/run/bridge.pair (one bridge per host).
const BRIDGE_MARK_DIR = '/agentbridge-dsh/run/bridge.pair'
...
async function deriveCwd(ctx, session) {
  if (session && session.meta && typeof session.meta.cwd === 'string') return session.meta.cwd
  try {
    const ws = ctx.get('workspaceRegistry')
    const list = ws && typeof ws.list === 'function' ? ws.list() : []
    const w = list.find((x) => x && typeof x.path === 'string')
...
async function bridgePairName(ctx, home) {
  try {
    const r = await safeRead(ctx, home + BRIDGE_MARK_DIR)
...
async function findPair(ctx, home, fullCwd) {
  const root = home + STATE_DIR
  const entries = await safeList(ctx, root)
  // This DSH instance's bridged pair (written by `abg dsh --pair <name>`)
  // is the strongest signal for which pair the panel must show. Then a live
  // pair of the exact session workspace, then any live pair, then newest.
  const bridgeName = await bridgePairName(ctx, home)
  const candidates = []
  for (const e of entries) {
    ...
    const live = daemon !== null && daemon.phase === 'ready' && !!daemon.ports
    candidates.push({ name, dir, meta, daemon, live, t })
  }
  candidates.sort((a, b) => b.t - a.t)
  const pairNameOf = (c) => c.meta.pairName || c.name
  const bridged = bridgeName ? candidates.find((c) => pairNameOf(c) === bridgeName) : null
  const wsLive = fullCwd ? candidates.find((c) => c.live && c.meta.cwd === fullCwd) : null
  const anyLive = candidates.find((c) => c.live)
  const pick = bridged || wsLive || anyLive || candidates[0] || null
  if (!pick) return null
  return {
    name: pairNameOf(pick),
    threadId: pick.meta.threadId || null,
    cwd: pick.meta.cwd || null,
    status: pick.meta.status || null,
    phase: pick.daemon && pick.daemon.phase ? pick.daemon.phase : null,
    turn: pick.daemon ? !!pick.daemon.turnInProgress : null,
    rolloutPath: pick.meta.rolloutPath || null,
    updatedAt: pick.meta.updatedAt || null,
  }
}
...
async function snapshot(ctx, sessionId) {
  ...
  const cwd = await deriveCwd(ctx, session)
  const pair = await findPair(ctx, home, cwd)
